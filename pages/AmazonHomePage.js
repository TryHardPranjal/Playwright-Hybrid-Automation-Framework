...
const selectors = {
  ...
  menu: {
    canvas: '#hmenu-canvas',
    container: '#hmenu-canvas:visible #hmenu-content',
...
class AmazonHomePage {
  ...
  async clickAllMenu() {
    ...
    await this.page.locator(selectors.button.allMenu).click({force: true});

  }

  async searchProduct(product) {

    await this.page.locator(selectors.button.searchBox).fill(product);
  ...
  async verifyMenuSections(menuData) {
    ...
    const container = this.page.locator(selectors.menu.container);

    for (const menu of menuData) {
      const section = container.locator(selectors.menu.section(menu.section)
      );
